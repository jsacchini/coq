  ...
  | Lcase(annot,t,a,br) ->
	...
	let (cn,ids,body) = b in
          ...
          | Some body -> let body' =
	                   ...
	                   if body == body' then b else (cn,ids,Some body')
          | None -> (cn,ids,None) in
...
let rec occurence k kind lam =
  ...
  | Lcase(_,t,a,br) ->
      ...
      Array.iter (fun (_,ids,c) ->
        match c with
        | Some c -> r := occurence (k+Array.length ids) kind c && !r
        | None -> ()) br;
      !r
  | Lif (t, bt, bf) ->
...
let rec lambda_of_constr env sigma c =
  ...
  | Case(ci,t,_,a,branches) ->  (* TODO: process indices -jls *)
	...
	if Int.equal arity 0 then (cn, empty_ids, b)
      ...
      let mk_branch_opt i b = match b with (* -jls *)
      | Some b' -> let (c,id,br) = mk_branch i b' in (c,id,Some br)
      | None -> ((ind,i+1),[||],None) (* no arguments -jls *) in
      let bs = Array.mapi mk_branch_opt branches in
